...
def find_value(param, param1):
    keys=['Garment_Type'.upper(),'Jenis Garment'.upper()]
    if str(param).upper() in keys or str(param1).upper() in keys:
        print("find it =============>&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
...
for file_name in file_names:
    ...
    for i in range(1, len(row1)):
        ...
        find_value(row2[i].value.strip(), row1[i].value.replace('*', '').strip())
    dicCount.append(i)
